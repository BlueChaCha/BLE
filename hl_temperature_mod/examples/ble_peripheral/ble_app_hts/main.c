...
#define DEVICE_NAME                     "heal2"                                /**< Name of device. Will be included in the advertising data. */
...
#define BATTERY_LEVEL_MEAS_INTERVAL1     APP_TIMER_TICKS(100, APP_TIMER_PRESCALER)  /**< Battery level measurement interval (ticks). */

#define MIN_BATTERY_LEVEL               40                                          /**< Minimum battery level as returned by the simulated measurement function. */
...
#define FIRST_CONN_PARAMS_UPDATE_DELAY  APP_TIMER_TICKS(500, APP_TIMER_PRESCALER)  /**< Time from initiating event (connect or start of indication) to first time sd_ble_gap_conn_param_update is called (5 seconds). */
...
#define APP_SGM_40561                   5
#define APP_NO_POWER_TIME               20
...
static ble_dfu_t 						m_dfus; //定义DFU结构体
...
static void temperature_measurement_send(int sw);
bool m_is_conn = false;
static uint16_t no_power_send = 0;	
static bsp_indication_t cur_ble_power = 	BSP_INDICATE_IDLE;																	 
static bsp_indication_t last_ble_state = 	BSP_INDICATE_IDLE;		
...
void dev_sgm40561_init(void)
{
	  nrf_gpio_cfg_sense_input(APP_SGM_40561, NRF_GPIO_PIN_PULLUP, NRF_GPIO_PIN_SENSE_LOW);
}

void dev_enter_sleep(void)
{
	  dev_max30205_uninit();
	  battery_uninit();
}

void dev_enter_work(void)
{
	  dev_max30205_uninit();
	  battery_uninit();
}
...
static void pm_evt_handler(pm_evt_t const * p_evt)
{
    ...
    switch (p_evt->evt_id)
    {
        ...
        case PM_EVT_CONN_SEC_SUCCEEDED:
        {
            ...
            {
                temperature_measurement_send(0);
 ...
 */
uint32_t sgm_state = 0;
static void battery_level_meas_timeout_handler(void * p_context)
{
		
    UNUSED_PARAMETER(p_context);
	 //读取充电状态，0为充电，1为未充电，充电时一直为0
	  sgm_state = nrf_gpio_pin_read(APP_SGM_40561);
		//如果是充电状态，则让LED进行充电状态的处理，常亮
	   //如果是未充电状态，则让LED根据当前最新的BLE状态进行处理
	  if(sgm_state == 0)
		{
			if( cur_ble_power != BSP_INDICATE_POWER )
			{
				bsp_indication_set(BSP_INDICATE_POWER);
				cur_ble_power = BSP_INDICATE_POWER;
		...
		else
		{
			if( cur_ble_power == BSP_INDICATE_POWER)
			{
				bsp_indication_set(last_ble_state);
				cur_ble_power = BSP_INDICATE_IDLE;
			}
...
//				no_power_send ++;
//			
//			if(no_power_send > APP_NO_POWER_TIME )
//			{
 ...
 */
static void hts_sim_measurement(ble_hts_meas_t * p_meas ,int sw)
{
...
//static int count = 0;
    ...
    uint32_t temp=0;
		...
		if(sw)
		{
	//	temp = dev_max30205_read_temperature();
		temp = dev_max30205_read_temperature();
		}
    ...
    p_meas->temp_in_celcius.mantissa = temp;
    p_meas->temp_in_fahr.exponent    = -2;
    p_meas->temp_in_fahr.mantissa    = (32 * 100) + ((0 * 9) / 5);
...
static void temperature_measurement_send(int sw)
{
    ...
    {
        hts_sim_measurement(&simulated_meas,sw);
...
static void sleep_mode_enter(void)
{
    ...
    //APP_ERROR_CHECK(err_code);
	  ...
	  dev_sgm40561_init();
    last_ble_state = BSP_INDICATE_IDLE;
    ...
    switch (ble_adv_evt)
    {
        case BLE_ADV_EVT_FAST:
            ...
            err_code = bsp_indication_set(BSP_INDICATE_ADVERTISING);
				    last_ble_state = BSP_INDICATE_ADVERTISING;
            APP_ERROR_CHECK(err_code);
            break;
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            ...
            APP_ERROR_CHECK(err_code);
            m_is_conn = true;
				    last_ble_state = BSP_INDICATE_CONNECTED;
            m_conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
				    no_power_send = 0;
				//    application_timers_start();
					app_timer_stop(m_battery_timer_id);
					app_timer_start(m_battery_timer_id, BATTERY_LEVEL_MEAS_INTERVAL1, NULL);
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
	...
	ble_dfu_on_ble_evt(&m_dfus, p_ble_evt);//添加DFU任务处理
}
...
int main(void)
{
    ...
    //APP_ERROR_CHECK(err_code);
    dev_max30205_powerCfg();//MAX30205 power off
...
//dev_max30205_init();
    ...
    for (;;)
    {
			...
			if(conflag==3)
			{
				...
				dev_max30205_init();
				power_manage();
				temperature_measurement_send(1);
				nrf_gpio_pin_clear(MAX30205_PWR_PIN);
				dev_max30205_uninit();
				if(m_is_conn)
				{
					...
					{
						fcnt++;
						app_timer_start(m_battery_timer_id, APP_TIMER_TICKS(500, APP_TIMER_PRESCALER), NULL);
					}
					else
						app_timer_start(m_battery_timer_id, BATTERY_LEVEL_MEAS_INTERVAL, NULL);
				}
